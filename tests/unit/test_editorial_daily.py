...
SAMPLE = {
    "strategy": "match_winner_wc_v1", "side_criterion": "blend", "kelly_fraction": 0.25,
    "date": "2026-06-20", "source": "polymarket", "generated_at": "2026-06-20T07:00:00",
    "rows": [
        {"home": "Netherlands", "away": "Sweden", "kickoff": "2026-06-20T15:00:00+00:00",
         "phase": "Group Stage", "pick_side": "HOME_WIN", "pick_team": "Netherlands",
        ...
        {"home": "Ecuador", "away": "Curaçao", "kickoff": "2026-06-20T14:00:00+00:00",
         "phase": "Group Stage", "pick_side": "HOME_WIN", "pick_team": "Ecuador",
...
def test_tweet_under_limit_and_clean():
    ...
    assert len(t) <= 280
    assert "Netherlands" in t            # el de mayor edge positivo
    assert "Ecuador" not in t            # descartado, no entra
...
def test_compute_returns_rows(seeded_data_root):
    # seeded_data_root tiene m1 (ARG vs FRA) + cuota polymarket para m1.
    date = (utcnow() + timedelta(hours=5)).strftime("%Y-%m-%d")
    data = daily_suggestions.compute(date, "fifa_world_cup_2026")
    assert data["strategy"] == "match_winner_wc_v1"
    assert len(data["rows"]) >= 1
    row = data["rows"][0]
    assert row["home"] == "Argentina"
