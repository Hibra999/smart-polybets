...
def test_ligamx_draft_requires_explicit_observation():
    assert load_active_strategy("liga_mx_2026") is None
    s = load_active_strategy("liga_mx_2026", require_approved=False)
    assert s is not None and not s.is_approved
    assert s.strategy_id == "match_winner_ligamx_v1"
    assert s.side_criterion == "blend"
    assert s.blend_weight == Decimal("0.5")
    assert s.warmup_match_no == 3
    assert s.use_bayes_filter is False
    assert s.kelly_fraction == Decimal("0.25")
    assert s.outcomes == ["HOME_WIN", "AWAY_WIN"]


def test_ligamx_strategy_parses():
    s = load_strategy_file("liga_mx_2026/strategies/match_winner_ligamx_v1")
    assert s.edge_threshold_auto == Decimal("0.10")
    assert s.outcomes == ["HOME_WIN", "AWAY_WIN"]
