"""Modelo Poisson de goles por equipo (básico) para el Mundial.
...
Con datos finos (un Mundial tiene pocos partidos por equipo) las tasas se encogen
...
Puro: no toca la DB. La siembra de partidos la hace el pipeline (wc_poisson_pipeline).
"""
...
class PoissonGoalsModel:
    ...
    `neutral=True` (default para Mundial): NO aplica ventaja local. Los partidos de
    Copa del Mundo son en sede neutral; el cociente goles_local/goles_visita de los
    datos refleja que el equipo listado primero suele ser el más fuerte (artefacto
    de orden), no una ventaja de cancha. Aplicarlo sesgaría los goles al primer
    equipo. Con sedes reales (liga), `neutral=False` sí usa el home_factor estimado.
    """
    shrink_k: float = 5.0          # pseudo-partidos al promedio (encoge tasas finas)
    neutral: bool = True           # sede neutral: ignora ventaja local
    ...
    def forecast(self, home: str, away: str) -> GoalsForecast:
        ...
        # sede neutral (Mundial): sin ventaja local. hf=1 reparte parejo.
